<?
  ...
  $string = "This is *my section of *text, the *missing words are *prefixed with *an asterik";
  ...
  while($word_count!=count($new_array)){
	...
	if(strpos($new_array[$word_count],"*")!==0){
		...
		$d = draw_rectangle($x_cord,$y_cord-8,10,(0-$t->getWidth(substr($new_array[$word_count],1,strlen($new_array[$word_count])-1))),100,0X00,0X00,0X00);	
